    ...
    importMethod(methodName, file, pluginState) {
        if (!pluginState.selectedMethods[methodName]) {
            ...
            const transformedMethodName = this.camel2UnderlineComponentName // eslint-disable-line
                ? util_1.camel2Underline(methodName)
                : this.camel2DashComponentName
                    ? util_1.camel2Dash(methodName)
                    : methodName;
            const path = util_1.winPath(this.customName ? this.customName(transformedMethodName) : path_1.join(this.libraryName, libraryDirectory, transformedMethodName, this.fileName) // eslint-disable-line
            );
            pluginState.selectedMethods[methodName] = this.transformToDefaultImport // eslint-disable-line
                ? helper_module_imports_1.addDefault(file.path, path, { nameHint: methodName })
                : helper_module_imports_1.addNamed(file.path, methodName, path);
            if (style === true) {
                helper_module_imports_1.addSideEffect(file.path, `${path}/style`);
            }
            else if (style === 'css') {
                helper_module_imports_1.addSideEffect(file.path, `${path}/style/css`);
            }
            else if (typeof style === 'function') {
                const stylePath = style(path, file);
                if (stylePath) {
                    helper_module_imports_1.addSideEffect(file.path, stylePath);
                }
            }
        }
