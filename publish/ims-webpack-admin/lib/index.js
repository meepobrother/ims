...
class ImsWebpackAdmin extends ims_webpack_1.ImsWebpack {
    ...
    onInit() {
        ...
        this.config.plugin('reactDll').use(webpack_1.DllReferencePlugin, [{
                manifest: require(path_1.join(root, 'attachment/template/library/react.manifest.json'))
            }]);
        this.config.plugin('polyfillDll').use(webpack_1.DllReferencePlugin, [{
                manifest: require(path_1.join(root, 'attachment/template/library/polyfill.manifest.json'))
            }]);
        this.config.plugin('coreDll').use(webpack_1.DllReferencePlugin, [{
                manifest: require(path_1.join(root, 'attachment/template/library/core.manifest.json'))
            }]);
        this.config.plugin('momentDll').use(webpack_1.DllReferencePlugin, [{
                manifest: require(path_1.join(root, 'attachment/template/library/moment.manifest.json'))
            }]);
        this.config.plugin('antdDll').use(webpack_1.DllReferencePlugin, [{
                manifest: require(path_1.join(root, 'attachment/template/library/antd.manifest.json'))
            }]);
        this.config.plugin('bizchartsDll').use(webpack_1.DllReferencePlugin, [{
                manifest: require(path_1.join(root, 'attachment/template/library/bizcharts.manifest.json'))
            }]);
        this.config.plugin('dataSetDll').use(webpack_1.DllReferencePlugin, [{
                manifest: require(path_1.join(root, 'attachment/template/library/dataSet.manifest.json'))
            }]);
