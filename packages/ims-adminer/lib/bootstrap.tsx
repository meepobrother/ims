...
import { ImsRouter, Route } from './router'
import pathToRegexp from 'path-to-regexp'
export async function bootstrap(_routes: IRouter[]) {
    const routes = _routes.map(route => transformHandler(route))
    const router = new ImsRouter(routes);
    const route = await router.resolve('/adminer/manager/help');
    debugger;
}

function transformHandler(route: IRouter, parent: Route = null): Route {
    const r: Route = {};
    if (parent) {
        r.path = route.path.replace(parent.path as string + '/', '');
    } else {
        r.path = route.path;
    }
    r.name = route.name;
